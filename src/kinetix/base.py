...
class ReactionParameters:
    def __init__(self,substrate: str,product: str,km_fwd: float,km_back: float,kcat_fwd: float,kcat_back: float):
...
class Enzyme:
    ...
    def add_reaction_params(self,r: ReactionParameters):
...
class Experiment:
  ...
  # def reversible_mm(self,Vf_max,S,Km_forward,Vb_max,P,Km_back):
    def reversible_mm(self,enzyme: Enzyme ,substrate: Reactant,product: Reactant ,params: ReactionParameters):
    ...
    def iteration(self,reaction_graph: nx.DiGraph, source: Reactant ,dt: float):
    ...
    def run(self,steps,dt: float=0.1):
